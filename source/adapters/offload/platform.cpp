...
urPlatformGet(ur_adapter_handle_t *, uint32_t, uint32_t NumEntries,
  ...
  if (pNumPlatforms) {
    if (auto Res = offloadPlatformGetCount(pNumPlatforms))
      return offloadResultToUR(Res);
  }

  if (phPlatforms) {
    if (auto Res = offloadPlatformGet(
            NumEntries,
            reinterpret_cast<offload_platform_handle_t *>(phPlatforms))) {
      return offloadResultToUR(Res);
    }
  }
...
urPlatformGetInfo(ur_platform_handle_t hPlatform, ur_platform_info_t propName,
  ...
  if (pPropSizeRet) {
    if (auto Res = offloadPlatformGetInfoSize(
  ...
  if (pPropValue) {
    if (auto Res = offloadPlatformGetInfo(
            reinterpret_cast<offload_platform_handle_t>(hPlatform), olInfo,
            propSize, pPropValue)) {
      return offloadResultToUR(Res);
    }
